...
| Date | Item |
|------|------|
| 2026-08 | **Preview APK ship from Actions** — `workflow_dispatch` on `main` starts EAS `preview` (cloud APK), waits, atomically replaces the nginx-served file. Self-hosted runner on the APK host; setup in `docs/shipping-preview-apk.md`. |
