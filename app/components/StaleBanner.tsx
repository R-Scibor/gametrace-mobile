...
export function formatLastSync(ts: number, now: Date = new Date()): string {
    const d = new Date(ts);
    const sameDay =
    ...
    const date = d.toLocaleDateString('pl', { day: '2-digit', month: 'short' });
    return `${date}, ${time}`;
}
    ...
    return (
        <View style={[styles.container, style]}>
            ...
            <Text style={styles.text}>
                Dane offline · ostatnia synchronizacja {formatLastSync(lastSyncTime)}
            </Text>
        </View>
    );
}
